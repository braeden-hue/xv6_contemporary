...
short task 10개·long task 2개의 고정 도착 스케줄에서 RR·SelectOnly·Preempt를 비교했다. 우선순위 기반 두 정책 모두 short 응답 median을 **2→1 tick**으로 낮췄지만, makespan·long 작업 완료 지연에 대한 사전 비용 기준은 매 라운드 충족하지 못했다. 결과 수집 방식 자체가 다른 작업의 실행을 지연시킬 수 있음을 확인해 수집 조건(V/VD/QD)을 별도로 대조했고, `-icount` 결정론적 조건에서 3회 재현성도 확인했다.

> 우선순위 정책은 짧은 작업의 응답시간을 개선했지만, 그 대가로 완료 지연·처리량 비용이 함께 증가했다.
